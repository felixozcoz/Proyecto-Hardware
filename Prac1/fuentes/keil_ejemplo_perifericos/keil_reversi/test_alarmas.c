...
const uint32_t retardo_periodico_10ms  = 0x8000000A;
// los retardos son de 100ms
const uint32_t retardo_cancelar = 0;
const uint32_t retardo_periodica_infinito = 0xA8000000;
...
void test_alarmas(void){
		...
		#if TEST_PERIODICA
				// programar alarma periódica con 10 ms de retardo
				// objetivo: disparar evento
			alarma_activar(EVENTO_FICTICIO_2, retardo_periodico_10ms, auxData_test);	
		#endif
			...
			alarma_activar(EVENTO_FICTICIO_3, retardo_cancelar, auxData_test);
		#endif
